...
from irs_app.models import IRSIndexedUrl, XMLUrlIndexer, ZIP_NGO
from irs_app.services import IrsScraper
...
def scrape_irs(url):
    try:
        ...
        if not data["organization_name"]:
            return
        ngo_data = ZIP_NGO.objects.create(
            **data
        )  
        ...
        log.error(f"SKIPPING ERROR: {url} - {str(e)}")
        IRSIndexedUrl.objects.filter(url=url).update(is_scraped=True, scraped_on=now())
...
def task_orchestrator():
    first_10 = IRSIndexedUrl.objects.filter(
                    is_scraped = False, 
                    locked=False,
                    trial__lt=5
                    )[:45]
