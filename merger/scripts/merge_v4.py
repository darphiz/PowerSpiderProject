...
class Merge:
    def __init__(self, ein) -> None:
        ...
        self.all_xml_eins = IrsXML_NGO.objects.filter(govt_reg_number=ein)
    ...
    def get_organization_name(self):
        sources = [
            ('GuidestarNGO', 'guidestar_ngo'),
            ('CharityNavigatorNGO', 'charitynavigator_ngo'),
            ('PledgeNGO', 'pledge_ngo'),
            ('IrsXML_NGO', 'irsxml_ngo'),
            ('IrsZIP_NGO', 'irszip_ngo'),
            ('GuideStarIndiaNGO', 'guidestarindia_ngo'),
            ('FCR_NGO', 'fcr_ngo'),
        ]
        for source_name, attr_name in sources:
            if isinstance(source_name, IrsXML_NGO):
                source = special_case_org_name(self.ein, self.all_xml_eins)
                if source:
                    self.domain_list.append(source.domain)
                    self.urls_scraped_list.append(source.urls_scraped)
                    return source.organization_name
            source = getattr(self, attr_name)
            if source:
                org_name = getattr(source, 'organization_name', None)
                if org_name:
                    self.domain_list.append(getattr(source, 'domain', None))
                    self.urls_scraped_list.append(getattr(source, 'urls_scraped', None))
                    return org_name
        return None
